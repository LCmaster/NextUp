<script lang="ts">
	...
	let inProgressTickets = $derived(
		tickets.filter((t) => t.status === 'in_progress' && !t.parent_id)
	);
	...
	<div class="max-w-full">
		...
		{#snippet ticketCard(ticket: Ticket, depth: number)}
			...
			<div
				onclick={(e) => {
					e.stopPropagation();
					if (editingTicketId !== ticket.id) toggleTicketExpanded(ticket.id);
				}}
			...
			>
				...
				{:else}
					...
					<div class="mb-2 flex items-start justify-between">
						<h4
							class="text-sm font-medium text-surface-900 dark:text-white {ticket.status ===
							'archived'
								? 'truncate'
								: ''}"
						>
						...
						<div class="flex gap-1 opacity-0 transition-opacity group-hover:opacity-100">
							{#if ticket.status === 'archived'}
								<button
									onclick={(e) => {
										e.stopPropagation();
										moveTicket(ticket, getLeftStatus(ticket.status));
									}}
					...
					{#if ticket.description && ticket.status !== 'archived'}
						<p
							class="mb-3 text-xs text-surface-700/50 dark:text-surface-200/30 whitespace-pre-wrap break-words {expandedTickets.includes(
								ticket.id
							)
								? ''
								: 'line-clamp-2'}"
							...
							<div class="flex gap-1">
								...
								{#if ticket.status !== 'archived'}
									<button
										onclick={(e) => {
											e.stopPropagation();
											moveTicket(ticket, getRightStatus(ticket.status));
										}}
										class="rounded p-1 text-surface-700/30 transition-colors hover:bg-surface-200 hover:text-surface-700 dark:text-surface-200/20 dark:hover:bg-surface-800 dark:hover:text-surface-200"
										title="Move right"
									>
					...
					<div class="mb-4 flex items-center justify-between">
						<div class="flex items-center gap-2">
							...
							<span class="text-xs text-surface-700/40 dark:text-surface-200/30"
								>{getColumnTickets(status).length}</span
							>
						</div>
					</div>
					<div class="space-y-3">
...
{:else}
	<div class="flex min-h-[50vh] items-center justify-center">
		<p class="text-sm text-surface-700/60 dark:text-surface-200/40 animate-pulse">
			Loading project...
		</p>
	</div>
{/if}
